...
if __name__ == '__main__':
    import json
    import pprint
    #{ "IMG_NAME" : {"html", 'tag_len': 44, 'cell_len_max': 11, 'width': 238, 'height': 59, 'type': 'simple' } 
    with open('sample_gt.json') as fp:
        true_json = json.load(fp)
        #print(true_json[list(true_json.keys())[0]])
        
    with open('sample_pred.json') as fp:
        #{"IMG_NAME" : "html string"}
        pred_json = json.load(fp)
        #print(list(pred_json.keys()))
        #print(pred_json[list(pred_json.keys())[0]])
